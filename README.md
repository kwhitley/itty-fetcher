...
<!-- BEGIN SNIPPET -->
```ts
let fetcher=(e,s)=>{let t="string"==typeof e?{base:e,...s}:e||{},{base:a=globalThis.location?.origin||"",headers:r={},...o}=t;return new Proxy((()=>{}),{get:(e,s)=>(...e)=>(async(e,s,t,a,r,o=("string"==typeof s[0]?s.shift():""),n=("GET"!=e?s.shift():null))=>{let i=new URL(o,o.includes("://")?void 0:a||globalThis.location?.href),l={...t,...s.shift(),method:e},f=new Headers(r),c=l.parse??"json";for(let e in l.query||{})i.searchParams.append(e,l.query[e]);if(n){let e="string"==typeof n;l.body=0==l.encode||e?n:JSON.stringify(n),!e&&0!=l.encode&&f.set("content-type","application/json")}for(let[e,s]of new Headers(l.headers||[]))f.set(e,s);let g=await(l.fetch||fetch)(new Request(i,{...l,headers:f})),h=g.ok?void 0:Object.assign(new Error(g.statusText),{status:g.status,response:g});if(c)try{g=g=await g[c](),h&&("json"===c?(h={...h,...g},h.message=g.message??h.message):h.message=g??h.message)}catch(e){!h&&(h=Object.assign(new Error(e.message),{status:g.status,response:g}))}for(let e of l.after||[]){let s=await e(g);null!=s&&(g=s)}if(l.tuple)return[h,h?void 0:g];if(h)throw h;return g})(s.toUpperCase(),e,o,a,r)})};
```
<!-- END SNIPPET -->
