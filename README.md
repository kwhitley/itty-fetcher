...
<!-- BEGIN SNIPPET -->
```ts
let fetcher=(e,s)=>{let t="string"==typeof e?{base:e,...s}:e||{};return new Proxy((()=>{}),{get:(e,s)=>(...e)=>(async(e,s,t,a=s.shift()??"",r=("GET"!=e?s.shift():null))=>{a=new URL((a.includes("://")?a:(t.base?.includes?.("://")?t.base:globalThis.location?.href+"/"+(t.base??""))+(a?"/"+a:"")).replace(/\/+/g,"/"));let n={...t,...s.shift(),method:e},o=new Headers(t.headers||{}),i=n.parse??"json",f="string"==typeof r;for(let e in n.query||{})a.searchParams.append(e,n.query[e]);r&&(n.body=0==n.encode||f?r:JSON.stringify(r),!f&&0!=n.encode&&o.set("content-type","application/json"));for(let[e,s]of new Headers(n.headers||[]))o.set(e,s);let l=await(n.fetch||fetch)(new Request(a,{...n,headers:o})),c=l.ok?void 0:Object.assign(new Error(l.statusText),{status:l.status,response:l});if(i)try{l=l=await l[i](),c&&("json"===i?(c={...c,...l},c.message=l.message??c.message):c.message=l??c.message)}catch(e){!c&&(c=Object.assign(new Error(e.message),{status:l.status,response:l}))}for(let e of n.after||[]){let s=await e(l);null!=s&&(l=s)}if(n.array)return[c,c?void 0:l];if(c)throw c;return l})(s.toUpperCase(),e,t)})};
```
<!-- END SNIPPET -->
