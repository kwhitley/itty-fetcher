...
<!-- BEGIN SNIPPET -->
```ts
let fetcher=(e,s)=>{let t="string"==typeof e?{base:e,...s}:e||{};return new Proxy((()=>{}),{get:(e,s)=>(...e)=>(async(e,s,t,a="",r,n=s.shift()??"",o=("GET"!=e?s.shift():null))=>{n=new URL(n.includes("://")?n:(a.includes("://")?a:globalThis.location?.href+a).replace(/\/$/,"")+(n?"/"+n.replace(/^\//,""):""));let i={...t,...s.shift(),method:e},l=new Headers(r),c=i.parse??"json",f="string"==typeof o;for(let e in i.query||{})n.searchParams.append(e,i.query[e]);o&&(i.body=0==i.encode||f?o:JSON.stringify(o),!f&&0!=i.encode&&l.set("content-type","application/json"));for(let[e,s]of new Headers(i.headers||[]))l.set(e,s);let p=await(i.fetch||fetch)(new Request(n,{...i,headers:l})),u=p.ok?void 0:Object.assign(new Error(p.statusText),{status:p.status,response:p});if(c)try{p=p=await p[c](),u&&("json"===c?(u={...u,...p},u.message=p.message??u.message):u.message=p??u.message)}catch(e){!u&&(u=Object.assign(new Error(e.message),{status:p.status,response:p}))}for(let e of i.after||[]){let s=await e(p);null!=s&&(p=s)}if(i.array)return[u,u?void 0:p];if(u)throw u;return p})(s.toUpperCase(),e,t,t.base,t.headers||{})})};
```
<!-- END SNIPPET -->
