...
<!-- BEGIN SNIPPET -->
```ts
let fetcher=(e,s,r=("string"==typeof e?{base:e,...s}:e||{}),{base:n=("undefined"!=typeof window?window?.location?.origin??"":""),headers:o={},...a}=r)=>new Proxy(((...e)=>t(...e)),{get:(t,e)=>t[e]??((...t)=>(async(t,e,s,r,n)=>{let o="string"==typeof e[0]?e.shift():"",a="get"!=t?e.shift():null;s={...s,...e.shift(),method:t},n=new Headers(n);let i=-1===o.indexOf("http")?r.endsWith("/")&&o.startsWith("/")?r+o.slice(1):r+o:o;i&&(i.startsWith("http")||r.startsWith("http"))||(i="http://localhost"+(i.startsWith("/")?i:"/"+i));let h=new URL(i);Object.entries(s.query||{}).forEach((([t,e])=>h.searchParams.append(t,e))),a&&(!1===s.encode?s.body=a:(s.body="string"==typeof a?a:JSON.stringify(a),"string"!=typeof a&&n.set("content-type","application/json")));for(let[t,e]of[...new Headers(s.headers??[])])n.set(t,e);s.headers=n;let f,d=new Request(h,s),c=await(s.fetch??fetch)(d);if(c.ok||(f=Object.assign(new Error(c.statusText),{status:c.status})),!1!==s.parse&&(c=await(c.headers.get("content-type")?.includes("json")?c.json():c.text())),f)return s.onError?s.onError(f,c):Promise.reject(f);for(let t of s.after||[])c=await t(c,d)??c;return c})(e,t,a,n,o))}),e=t();
```
<!-- END SNIPPET -->
