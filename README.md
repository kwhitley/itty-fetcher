...
<!-- BEGIN SNIPPET -->
```ts
let fetcher=(e,s)=>{let t="string"==typeof e?{base:e,...s}:e||{};return new Proxy((()=>{}),{get:(e,s)=>(...e)=>(async(e,s,t,a="",r=s.shift()??"",n=("GET"!=e?s.shift():null))=>{r=new URL((r.includes("://")?r:(a.includes("://")?a:globalThis.location?.href+"/"+a)+(r?"/"+r:"")).replace(/\/+/g,"/"));let o={...t,...s.shift(),method:e},i=new Headers(t.headers||{}),c=o.parse??"json",l=n.charAt;for(let e in o.query||{})r.searchParams.append(e,o.query[e]);n&&(o.body=0==o.encode||l?n:JSON.stringify(n),!l&&0!=o.encode&&i.set("content-type","application/json"));for(let[e,s]of new Headers(o.headers||[]))i.set(e,s);let f=await(o.fetch||fetch)(new Request(r,{...o,headers:i})),h=f.ok?void 0:Object.assign(new Error(f.statusText),{status:f.status,response:f});if(c)try{f=f=await f[c](),h&&("json"===c?(h={...h,...f},h.message=f.message??h.message):h.message=f??h.message)}catch(e){!h&&(h=Object.assign(new Error(e.message),{status:f.status,response:f}))}for(let e of o.after||[]){let s=await e(f);null!=s&&(f=s)}if(o.array)return[h,h?void 0:f];if(h)throw h;return f})(s.toUpperCase(),e,t,t.base)})};
```
<!-- END SNIPPET -->
