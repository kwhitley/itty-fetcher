...
<!-- BEGIN SNIPPET -->
```ts
let fetcher=(e,s)=>{let t="string"==typeof e?{base:e,...s}:e||{},{base:a=globalThis.location?.origin||"",headers:r={},...o}=t;return new Proxy((()=>{}),{get:(e,s)=>(...e)=>(async(e,s,t,a,r,o=s.shift()??"",n=("GET"!=e?s.shift():null))=>{let i=new URL(o,o.includes("://")?void 0:a||globalThis.location?.href),l={...t,...s.shift(),method:e},f=new Headers(r),c=l.parse??"json",g="string"==typeof n;for(let e in l.query||{})i.searchParams.append(e,l.query[e]);n&&(l.body=0==l.encode||g?n:JSON.stringify(n),!g&&0!=l.encode&&f.set("content-type","application/json"));for(let[e,s]of new Headers(l.headers||[]))f.set(e,s);let h=await(l.fetch||fetch)(new Request(i,{...l,headers:f})),d=h.ok?void 0:Object.assign(new Error(h.statusText),{status:h.status,response:h});if(c)try{h=h=await h[c](),d&&("json"===c?(d={...d,...h},d.message=h.message??d.message):d.message=h??d.message)}catch(e){!d&&(d=Object.assign(new Error(e.message),{status:h.status,response:h}))}for(let e of l.after||[]){let s=await e(h);null!=s&&(h=s)}if(l.array)return[d,d?void 0:h];if(d)throw d;return h})(s.toUpperCase(),e,o,a,r)})};
```
<!-- END SNIPPET -->
